...
class BacktestingEngine:
    ...
    def run(self, **config) -> BacktestResult:
        ...
        if record_backtest_details:
            ...
            nav_base = float(initial_capital) if len(portfolio_values) else 1.0

            nav_df = pd.DataFrame(
                {
                    "Date": common_index,
                    "RiskOnAlloc": [round(float(v) * 100.0, 2) for v in allocation.values],
                    "RiskOffAlloc": [
                        round(float((1.0 - v) * 100.0), 2) for v in allocation.values
                    ],
                    "NAV": [round((float(pv) / nav_base) * 100.0, 2) for pv in portfolio_values],
                    "PortfolioValue": [round(float(pv), 2) for pv in portfolio_values],
                }
            )
