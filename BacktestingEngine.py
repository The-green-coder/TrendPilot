...
class BacktestingEngine:
    ...
    def run(self, **config) -> BacktestResult:
        ...
        if record_backtest_details:
            ...
            nav_base = float(portfolio_values.iloc[0]) if len(portfolio_values) > 0 else float(initial_capital)
            nav_values = [round((float(pv) / nav_base) * 100.0, 2) for pv in portfolio_values]
            portfolio_value_values = [round(float(pv), 2) for pv in portfolio_values]

            nav_df = pd.DataFrame(
                {
                    "Date": common_index,
                    "RiskOnAlloc": [round(v * 100.0, 2) for v in allocation.values],
                    "RiskOffAlloc": [round((1.0 - v) * 100.0, 2) for v in allocation.values],
                    "NAV": nav_values,
            ...
            def _append_trade(dt, iteration, instrument, action, price, qty, trade_value):
                ...
                qty_rounded = round(float(qty), 2)
                txn_cost_rounded = round(float(txn_cost_amount), 2)
                slippage_rounded = round(float(slippage_amount), 2)
