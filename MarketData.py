...
def _normalize_date_column(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Reset the index and try to name the resulting column "date" in one step
    try:  # pandas >= 1.3 supports ``names``
        df = df.reset_index(names="date")
    except TypeError:  # pragma: no cover - legacy pandas path
        df = df.reset_index()

    # Harmonize common index column names that may appear
    index_name = getattr(getattr(df, "index", None), "name", None) or ""
    rename_candidates = {
        "index": "date",
        "Date": "date",
        "Datetime": "date",
    ...
    # Handle auto-generated unnamed columns (e.g., "Unnamed: 0") from legacy CSVs
    for col in list(df.columns):
        if col.startswith("Unnamed") and "date" not in df.columns:
            df.rename(columns={col: "date"}, inplace=True)

    # If we still lack a ``date`` column, promote the current index values
    if "date" not in df.columns:
        df["date"] = df.index
        df.reset_index(drop=True, inplace=True)

    # Absolute last resort: if the above logic still failed to create a
    # ``date`` column (e.g., legacy CSVs with no recognizable index), fall back
    # to a simple increasing range so downstream code can still index safely.
    if "date" not in df.columns:
